...
async function fetchMonsterGigs() {
  ...
  try {
    ...
    const jobsRes = await axios.get(
      'https://api.monster.io/adtech-job-distributor/v1/feed',
      {
    ...
    if (typeof jobsRes.data === 'string' && jobsRes.data.trim().startsWith('<?xml')) {
      ...
      try {
        ...
        let jobfeed = parsed && parsed.jobfeed;
        ...
        console.error('[Monster] Failed to parse XML:', err && err.message ? err.message : err);
        return [];
      }
    } else {
      ...
      if (typeof jobsRes.data === 'string') {
        try {
          ...
          console.error('[Monster] Failed to parse JSON:', err && err.message ? err.message : err);
          return [];
        }
      } else {
    ...
    console.error('[Monster] Fetch error:', err && err.message ? err.message : err);
    return [];
  }
}

module.exports = { fetchMonsterGigs }; 