import React from 'react';
import type { Meta, StoryObj } from '@storybook/react';
import { PerformanceExport } from './PerformanceExport.tsx';

const meta: Meta<typeof PerformanceExport> = {
  title: 'Dev/PerformanceExport',
  component: PerformanceExport,
  parameters: {
    layout: 'centered',
  },
  tags: ['autodocs'],
};

export default meta;
type Story = StoryObj<typeof PerformanceExport>;

const generateMockMetrics = (count: number) => {
  const metrics = [];
  const now = Date.now();
  const hourMs = 60 * 60 * 1000;

  for (let i = 0; i < count; i++) {
    const timestamp = new Date(now - (count - i) * hourMs);
    metrics.push(
      {
        componentName: 'TestComponent',
        metricName: 'renderTime',
        value: 50 + Math.random() * 20,
        timestamp,
      },
      {
        componentName: 'TestComponent',
        metricName: 'updateTime',
        value: 30 + Math.random() * 15,
        timestamp,
      },
      {
        componentName: 'TestComponent',
        metricName: 'frameTime',
        value: 16 + Math.random() * 5,
        timestamp,
      },
      {
        componentName: 'TestComponent',
        metricName: 'memoryUsage',
        value: 100 * 1024 * 1024 + Math.random() * 50 * 1024 * 1024,
        timestamp,
      }
    );
  }

  return metrics;
};

export const Default: Story = {
  args: {
    metrics: generateMockMetrics(24),
  },
};

export const WithComponentName: Story = {
  args: {
    metrics: generateMockMetrics(24),
    componentName: 'TestComponent',
  },
};

export const EmptyMetrics: Story = {
  args: {
    metrics: [],
  },
};

export const DarkMode: Story = {
  args: {
    metrics: generateMockMetrics(24),
  },
  parameters: {
    themes: {
      defaultTheme: 'dark',
    },
  },
};
