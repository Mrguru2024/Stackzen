-- Row Level Security for public tables used with Supabase Auth + PostgREST.
-- Prisma User.id is a cuid; Supabase JWT auth.uid() is auth.users.id (UUID).
-- Policies use prisma_user_id_for_auth() which maps auth.uid() -> Prisma id via "User"."authUserId".
-- Idempotent with Prisma: column may already exist from prisma migrate.
...
-- ---------------------------------------------------------------------------
-- Helper: current JWT -> Prisma User.id (text)
-- ---------------------------------------------------------------------------
...
ALTER TABLE "WellnessScore" ENABLE ROW LEVEL SECURITY;
...
-- Optional one-time backfill on Supabase (run in SQL editor if needed):
-- UPDATE "User" u SET "authUserId" = au.id FROM auth.users au
-- WHERE u.email IS NOT NULL AND au.email IS NOT NULL AND lower(u.email) = lower(au.email) AND u."authUserId" IS NULL;
